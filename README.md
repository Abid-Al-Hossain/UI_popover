# UI Foundry Popover Studio

Standalone visual Popover generator built with Next.js, React, and TypeScript.

Standalone Popover component studio for building native, preview-honest, React-exportable UI patterns that match the UI Foundry product line.
...
- designers building Popover UI quickly
- developers who need editable React starting points
- marketplace buyers who want a self-contained Popover generator
...
The editor is focused on controls that are native to Popover and useful in real product work:
- trigger, open state, placement, alignment, offset, arrow, content, and dismissal intent
- surface, border, radius, shadow, color, typography, spacing, and motion-safe animation
- menu-like, info, form-light, profile, and contextual content archetypes
- popover semantics without claiming tooltip/dialog behavior incorrectly
- React export for popover shell

The goal is maximum useful depth without adding unrelated controls that would make the component stop reading as Popover.
...
This repo is intended to publish as Abid-Al-Hossain/UI_popover.
