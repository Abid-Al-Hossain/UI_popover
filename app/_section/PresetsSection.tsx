...
export default function PresetsSection({ activePresetId, onApply }: Props) {
  ...
  const surprise = () => {
    const source = filtered.length ? filtered : POPOVER_PRESETS;
    onApply(source[Math.floor(Math.random() * source.length)]);
  };

  return (
    <SectionCard title="Presets" subtitle="Structured full-state presets with search, filters, surprise, and applied-state highlighting.">
